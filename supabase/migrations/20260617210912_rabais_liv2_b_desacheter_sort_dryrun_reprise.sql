-- RABAIS liv 2 — migration B
-- desacheter_sort : + paramètre p_dry_run, + reprise du rabais Acquisition de Cercle.
-- DROP+recreate (patron fonction unique). Aucun appelant interne (vérifié s219).
-- Étiquette au format INSTANCE (migration A) => comptage de présence exact.

DROP FUNCTION IF EXISTS public.desacheter_sort(uuid);

CREATE OR REPLACE FUNCTION public.desacheter_sort(p_personnage_sort_id uuid, p_dry_run boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_nom text;
  v_cercle text;
  v_reprises jsonb := '[]'::jsonb;     -- public (donnees)
  v_appliquer jsonb := '[]'::jsonb;    -- interne (pc à traiter sur apply)
...
BEGIN
  ...
  v_nom := (SELECT nom FROM sorts WHERE id = v_ps.sort_id);
  v_cercle := (SELECT cercle FROM sorts WHERE id = v_ps.sort_id);
  ...
  FOR r IN
    SELECT pc.id AS pc_id, pc.niveau_acquis, pc.choix_achat, c.nom AS comp_nom,
           ...
           (SELECT count(*) FROM jsonb_array_elements(pc.rabais_items) it
              WHERE it->>'type' = 'sort'
                AND EXISTS (SELECT 1 FROM personnage_sorts ps2
                            WHERE ps2.id = (it->>'id')::uuid
                              AND ps2.personnage_id = v_ps.personnage_id)
           ) AS l_present
    FROM personnage_competences pc
    JOIN competences c ON c.id = pc.competence_id
    WHERE pc.personnage_id = v_ps.personnage_id
      AND pc.rabais_items @> jsonb_build_array(jsonb_build_object('type','sort','id',p_personnage_sort_id))
  LOOP
  ...
  v_net := v_ps.xp_depense - v_reprise_totale;
  v_xp_restant_apres := v_xp_restant_avant + v_net;
  v_bloque := (v_xp_restant_apres < 0);

  -- ===== GARDE ROUGE (dry-run ET apply) =====
  IF v_bloque THEN
    v_msg_action := 'Supprimer ce sort te ferait passer en XP négatif (il manque ' || (-v_xp_restant_apres) ||
                    ' XP). Retire d''abord une Acquisition de Cercle de niveau supérieur pour le cercle « ' ||
                    COALESCE(v_cercle,'?') || ' » — cela te rendra des XP — puis ce sort.';
    RETURN jsonb_build_object('succes', false,
      'erreurs', jsonb_build_array(jsonb_build_object('code','reprise_rouge','message', v_msg_action)),
      'avertissements', '[]'::jsonb,
      'donnees', jsonb_build_object('dry_run', p_dry_run, 'type','sort','nom', v_nom, 'cercle', v_cercle,
        'xp_rembourse', v_ps.xp_depense, 'reprises', v_reprises, 'reprise_totale', v_reprise_totale,
  ...
  IF p_dry_run THEN
    RETURN jsonb_build_object('succes', true, 'erreurs','[]'::jsonb, 'avertissements','[]'::jsonb,
      'donnees', jsonb_build_object('dry_run', true, 'type','sort','nom', v_nom, 'cercle', v_cercle,
        'xp_rembourse', v_ps.xp_depense, 'reprises', v_reprises, 'reprise_totale', v_reprise_totale,
  ...
  IF v_ps.xp_depense > 0 THEN
    UPDATE personnages SET date_modification = now(), updated_at = now() WHERE id = v_ps.personnage_id;
  ...
  -- Reprises : 1 mouvement depense_competence négatif + retrait d'une occurrence de l'étiquette
  ...
  LOOP
    ...
    SELECT v_ps.personnage_id, 'depense_competence', -1,
      'Reprise rabais ' || (r.item->>'comp') || ' niveau ' || (r.item->>'niveau') || ' (cercle ' || (r.item->>'choix') || ')',
    ...
    WHERE pc.id = (r.item->>'pc_id')::uuid
      AND t.elem->>'type' = 'sort' AND (t.elem->>'id')::uuid = p_personnage_sort_id
  ...
  IF public.doit_logger_action(v_perso.joueur_id) THEN
    PERFORM public.log_audit('personnage', v_perso.id, 'desacheter_sort', jsonb_build_object('personnage_sort_id', p_personnage_sort_id, 'nom', v_nom, 'niveau', v_ps.niveau_sort, 'xp_rembourse', v_ps.xp_depense, 'reprise_totale', v_reprise_totale));
  END IF;
