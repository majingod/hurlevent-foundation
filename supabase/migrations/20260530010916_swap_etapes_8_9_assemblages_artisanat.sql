-- =====================================================================
-- PR-A : Swap étapes 8 <-> 9
--   Position 8 = Assemblages de runes (était 9)
--   Position 9 = Artisanat (était 8)
-- Le dispatcher valider_etape() route par numéro de position : on échange
-- donc le CONTENU de valider_etape_8/_9 et les blocs CASE 8/9 d'annuler_etape.
-- Options préservées à l'identique (STABLE / SECURITY DEFINER / search_path).
-- =====================================================================

-- ---------------------------------------------------------------------
-- valider_etape_8 (NOUVEAU) = check runiste / assemblages
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- valider_etape_9 (NOUVEAU) = check artisanat (alchimie/forge/joaillerie)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- annuler_etape : blocs CASE 8 <-> 9 échangés (comptage + exécution + messages)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  CASE p_etape_courante
    ...
    -- >>> SWAP : position 8 = ASSEMBLAGES (ex-bloc WHEN 9) <<<
    WHEN 8 THEN
      SELECT COUNT(*), COALESCE(SUM(xp_depense), 0)
    ...
    -- >>> SWAP : position 9 = ARTISANAT (ex-bloc WHEN 8) <<<
    WHEN 9 THEN
      SELECT COUNT(*), COALESCE(SUM(xp_depense), 0)
  ...
  CASE p_etape_courante
    ...
    -- >>> SWAP : position 8 = ASSEMBLAGES (ex-bloc WHEN 9, message -> étape 8) <<<
    WHEN 8 THEN
      ...
      SELECT
        ...
        'remboursement',
        pa.xp_depense,
        format('Annulation étape 8 — assemblage « %s »', a.nom),
        v_uid,
        pa.assemblage_id
      FROM personnage_assemblages pa
      JOIN assemblages_runes a ON a.id = pa.assemblage_id
    ...
    -- >>> SWAP : position 9 = ARTISANAT (ex-bloc WHEN 8, messages -> étape 9) <<<
    WHEN 9 THEN
      -- Recettes
      ...
      SELECT
        ...
        'remboursement',
        pr.xp_depense,
        format('Annulation étape 9 — recette « %s »', r.nom),
        v_uid,
        pr.recette_id
      FROM personnage_recettes pr
      JOIN recettes_alchimie r ON r.id = pr.recette_id
      ...
      SELECT
        ...
        'remboursement',
        pof.xp_depense,
        format('Annulation étape 9 — objet de forge « %s »', o.nom),
        v_uid,
        pof.objet_id
      FROM personnage_objets_forge pof
      JOIN objets_forge o ON o.id = pof.objet_id
      ...
      SELECT
        ...
        'remboursement',
        poj.xp_depense,
        format('Annulation étape 9 — objet de joaillerie « %s »', o.nom),
        v_uid,
        poj.objet_id
      FROM personnage_objets_joaillerie poj
      JOIN objets_joaillerie o ON o.id = poj.objet_id
