-- Cascade désachat inter-compétences + mode aperçu (dry-run).
-- Remplace le refus (DEPENDANCES_PREREQUIS) par une cascade récursive : retire la compétence cible,
-- puis en boucle (point fixe) tous les dépendants devenus invalides (suppression chirurgicale par niveau via
-- niveau_max_achetable), et balaye les sorts/prières créés si un enabler (Acquisition de Sort/Prière) est atteint.
-- p_dry_run=true : renvoie l'aperçu complet (items_detail + xp_rembourse + cascade) SANS rien modifier.
...
BEGIN
  ...
  -- ETAPE 1 : SIMULATION (savepoint annule) pour calculer l'ensemble complet de la cascade
  BEGIN
  ...
  -- ETAPE 2 : detection des enablers magiques dans l'ensemble retire (lignes restaurees par le rollback)
  ...
  -- ETAPE 3 : construire l'apercu (items_detail + totaux) a partir des donnees live
  SELECT COALESCE(jsonb_agg(jsonb_build_object('type','competence','type_label','Compétence','nom',t.nom,'quantite',t.cnt,'xp_unitaire',t.xp_unit,'xp_total',t.xp_total) ORDER BY t.nom),'[]'::jsonb), COALESCE(SUM(t.xp_total),0)::int, COALESCE(SUM(t.cnt),0)::int, COUNT(*)::int
    INTO v_items_comp, v_xp_comp, v_nb_comp, v_nb_comp_distinct
    FROM (SELECT c.nom, count(*)::int cnt, SUM(pc.xp_depense)::int xp_total, MIN(pc.xp_depense)::int xp_unit FROM personnage_competences pc JOIN competences c ON c.id=pc.competence_id WHERE pc.id = ANY(v_removal_ids) GROUP BY c.nom) t;
