-- Fix : reparations_forge n'a pas de colonne nom → tri stable schema-agnostique.
-- Redéfinition complète (idempotente) de rpc_snapshot_hors_ligne().
...
BEGIN
  v_tables := jsonb_build_object(
    ...
    'reparations_forge', (SELECT coalesce(jsonb_agg(to_jsonb(t) ORDER BY to_jsonb(t)::text), '[]'::jsonb) FROM reparations_forge t WHERE t.est_actif),
