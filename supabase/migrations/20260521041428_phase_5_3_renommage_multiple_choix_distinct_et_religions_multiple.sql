-- =====================================================================
-- Migration Phase 5.3 : Renommage `multiple_langue` -> `multiple_choix_distinct`
-- + ouverture multi-achat pour Connaissances des Religions
-- =====================================================================
-- Contexte : Manuel des règles 2026 (édition 6 mai 2026) autorise
-- l'achat répété de Connaissances des Religions (une fois par religion
-- différente). La consécration reste unique et déjà gérée via
-- personnages.religion_id + est_croyant.
-- =====================================================================

-- 1. Drop ancien CHECK
ALTER TABLE competences DROP CONSTRAINT IF EXISTS competences_type_achat_check;

-- 2. Renommer toutes les compétences en multiple_langue (Décryptage, Langue supplémentaire)
UPDATE competences
SET type_achat = 'multiple_choix_distinct'
WHERE type_achat = 'multiple_langue';

-- 3. Basculer Connaissances des Religions vers multiple_choix_distinct
--    (auparavant unique_avec_choix -> 1 achat max ; désormais multi-achat
--    avec doublon bloqué sur la religion choisie)
UPDATE competences
SET type_achat = 'multiple_choix_distinct'
WHERE nom = 'Connaissances des Religions';

-- 4. Re-créer CHECK avec nouvelle valeur (multiple_langue retiré).
--    unique_avec_choix conservé pour usage futur même si actuellement orphelin.
ALTER TABLE competences ADD CONSTRAINT competences_type_achat_check
  CHECK (type_achat = ANY (ARRAY[
    'simple',
    'unique_avec_choix',
    'multiple_avec_choix_par_niveau',
    'multiple_choix_distinct',
    'multiple_sans_choix'
  ]));

-- 5. Mettre à jour peut_acheter_competence : renommer la branche
--    WHEN 'multiple_langue' THEN -> WHEN 'multiple_choix_distinct' THEN
