-- PR-A : autosave brouillon de l'étape 1 du wizard.
-- Ajoute p_brouillon (DEFAULT false, rétrocompatible). Quand true : persiste
-- les champs sans valider/avancer/logger → permet un autosave onBlur silencieux.
-- Le « Suivant » (p_brouillon=false par défaut) garde le comportement actuel.
CREATE OR REPLACE FUNCTION public.sauvegarder_etape_1(
  p_personnage_id uuid, p_nom text, p_gn_completes integer, p_mini_gn_completes integer,
  p_ouvertures_terrain integer, p_est_croyant boolean, p_religion_id uuid,
  p_historique text DEFAULT NULL::text, p_ame_personnage text DEFAULT NULL::text,
  p_brouillon boolean DEFAULT false)
...
BEGIN
  ...
  EXCEPTION WHEN check_violation THEN
    RETURN jsonb_build_object('succes', false,
      ...
      'avertissements', '[]'::jsonb, 'donnees', jsonb_build_object('personnage_id', p_personnage_id));
  END;
  -- Brouillon (autosave onBlur) : on persiste sans valider, sans avancer, sans logger.
  IF p_brouillon THEN
