-- s373 — Décision Fred : DÉGEL À LA CONFIRMATION DE PRÉSENCE. Une inscription 'present'
-- (photo déjà prise à la confirmation) ne gèle plus la fiche : seul 'en_attente' sur un
-- événement non clôturé maintient la fenêtre de gel. Le bandeau « fiche gelée jusqu'à la
-- confirmation des présences » devient exact. Le personnage confirmé passe en 'campagne'
-- (ajouts seulement) sans attendre la clôture ; la récompense arrive toujours à la clôture.
...
GRANT EXECUTE ON FUNCTION public.etat_edition_personnage(uuid) TO postgres, service_role, authenticated, anon;
