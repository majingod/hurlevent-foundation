-- ============================================================================
-- RPC : changer_classe_personnage  (Phase 2 M3bc) — donnees enrichies dry_run
-- Ajouts display-only : perdues.raison + niveaux[{niv,xp,gratuit}] ;
-- dormants.niveau ; bloc multi_choix (selecteur D6 + label option). Mecaniques inchangees.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_have_paid AND NOT v_have_free THEN
      ...
      IF v_grat.type_achat = 'multiple_choix_distinct' THEN
        -- options du selecteur (uniquement si plusieurs instances payees subsistent)
        IF (SELECT count(*) FROM personnage_competences pc
              WHERE pc.personnage_id=p_personnage_id AND pc.competence_id=v_grat.cid
                AND pc.xp_depense>0 AND pc.id <> ALL(v_removal_ids)) > 1 THEN
          v_multi_choix := v_multi_choix || jsonb_build_object(
            'competence_id', v_grat.cid, 'nom', v_grat.nom,
            'options', (SELECT jsonb_agg(jsonb_build_object('choix_achat',pc.choix_achat,'label',COALESCE((SELECT nom FROM langues l WHERE l.id::text=pc.choix_achat),(SELECT nom FROM religions r WHERE r.id::text=pc.choix_achat),pc.choix_achat),'xp',pc.xp_depense) ORDER BY pc.xp_depense, pc.choix_achat)
                        FROM personnage_competences pc
                        WHERE pc.personnage_id=p_personnage_id AND pc.competence_id=v_grat.cid
                          AND pc.xp_depense>0 AND pc.id <> ALL(v_removal_ids)),
            'defaut', (SELECT pc.choix_achat FROM personnage_competences pc
                        WHERE pc.personnage_id=p_personnage_id AND pc.competence_id=v_grat.cid
                          AND pc.xp_depense>0 AND pc.id <> ALL(v_removal_ids)
                        ORDER BY pc.xp_depense, pc.choix_achat LIMIT 1));
        END IF;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('nom',t.nom,'raison',t.raison,'niveaux',t.niveaux,'xp',t.xp) ORDER BY t.nom),'[]'::jsonb),
    ...
    SELECT c.nom,
           CASE
             WHEN c.id IN (
                  ...
                  EXCEPT
                  SELECT (g->>'competence_id')::uuid FROM jsonb_array_elements(COALESCE(v_classe.competences_gratuites,'[]'::jsonb)) g)
               THEN 'gratuite_obsolete'
             WHEN c.classes_requises IS NOT NULL AND NOT (v_norm_new = ANY(c.classes_requises))
               THEN 'class_locked'
             WHEN NOT c.est_general AND c.categorie <> v_norm_new AND c.classes_requises IS NULL AND max(pc.niveau_acquis) > 2
               THEN 'over_cap'
             ELSE 'cascade'
           END AS raison,
           jsonb_agg(jsonb_build_object('niv',pc.niveau_acquis,'xp',pc.xp_depense,'gratuit',(pc.xp_depense=0)) ORDER BY pc.niveau_acquis) AS niveaux,
           SUM(pc.xp_depense)::int AS xp
    FROM personnage_competences pc JOIN competences c ON c.id=pc.competence_id
    WHERE pc.id = ANY(v_removal_ids)
    GROUP BY c.id, c.nom, c.classes_requises, c.est_general, c.categorie
  ) t;

  -- dormants sorts/prieres (statut achete -> cree) + refund
  IF v_purge_sorts THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object('type','sort','nom',COALESCE(ps.nom_personnalise,s.nom),'niveau',ps.niveau_sort,'xp',ps.xp_depense) ORDER BY COALESCE(ps.nom_personnalise,s.nom)),'[]'::jsonb),
  ...
  IF v_purge_prieres THEN
    SELECT v_dormants || COALESCE(jsonb_agg(jsonb_build_object('type','priere','nom',COALESCE(pp.nom_personnalise,pr.nom),'niveau',pp.niveau_priere,'xp',pp.xp_depense) ORDER BY COALESCE(pp.nom_personnalise,pr.nom)),'[]'::jsonb),
