-- ============================================================
-- Lot 1b-2 : migrer les 6 RPC schema A restants vers creer_notification
-- Refactor NEUTRE (meme notif au mot pres). Idempotent (CREATE OR REPLACE).
-- ============================================================

-- 1) approuver_race_demande -------------------------------------------------
...
BEGIN
  ...
  PERFORM public.creer_notification(
    p_message := format('Niveau de « %s » ajusté de %s%s (niveau %s → %s)%s.',
      COALESCE(v_perso.nom,'Sans nom'),
