-- ============================================================
-- Bug #21 — Connaissances des Religions achetable plusieurs fois
-- ============================================================
-- Manuel : "Cette compétence peut être achetée plusieurs fois uniquement
-- afin d'acquérir la connaissance des rites et coutumes de différentes
-- religions". DB actuelle : type_achat = 'unique_avec_choix' (1 achat max).
-- Bascule vers 'multiple_langue' (patron Langue supplémentaire / Décryptage).
-- En passant, 2 messages de la branche multiple_langue sont génériqués
-- (étaient hardcodés "langue").
-- ============================================================

-- 1. Bascule type_achat (idempotent)
UPDATE competences
SET type_achat = 'multiple_langue'
WHERE nom = 'Connaissances des Religions'
  AND type_achat = 'unique_avec_choix';

-- 2. peut_acheter_competence : messages génériques dans la branche multiple_langue
...
BEGIN
  ...
  CASE v_competence.type_achat
    ...
    WHEN 'multiple_langue' THEN
      ...
      IF v_deja_choisi THEN
        RETURN jsonb_build_object('peut_acheter', false, 'raison', format('Vous avez déjà acquis "%s"', p_choix_achat));
      END IF;
