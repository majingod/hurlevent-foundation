...
-- (événement non clôturé), plus seulement au versement de la récompense. Elle capture la
-- fiche telle que jouée au GN, avant récompense et avant dégel. La dé-confirmation avant
-- clôture retire la photo (symétrie avec l'annulation post-récompense du CAS 3).
-- Conséquence assumée par Fred : une présence confirmée PENDANT la fenêtre de gel
-- photographie et (avec 20260803_degel) dégèle la fiche immédiatement.
...
BEGIN
  ...
  IF v_evt.id IS NULL OR NOT COALESCE(v_evt.est_termine, false) THEN
    ...
    -- Photo de composition à la CONFIRMATION (décision Fred s373) : la photo capture
    -- la fiche telle que jouée au GN, avant récompense et avant dégel. À la
    -- dé-confirmation (present -> autre) avant clôture, la photo n'a plus d'objet.
    IF p_nouveau_statut = 'present' AND v_ins.statut IS DISTINCT FROM 'present' THEN
      INSERT INTO public.personnage_compo_photos (personnage_id, evenement_id, inscription_id, compo, acteur_id)
      SELECT v_ins.personnage_id, v_ins.evenement_id, p_inscription_id,
             public.capturer_compo_personnage(v_ins.personnage_id), auth.uid()
      WHERE v_ins.personnage_id IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM public.personnage_compo_photos pcp WHERE pcp.inscription_id = p_inscription_id);
    ELSIF p_nouveau_statut <> 'present' AND v_ins.statut = 'present' THEN
...
-- CREATE OR REPLACE remet l'ACL EXECUTE à PUBLIC → re-verrouillage à l'identique (mesuré avant).
REVOKE ALL ON FUNCTION public.changer_statut_inscription(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.changer_statut_inscription(uuid, text) TO postgres, service_role, authenticated;
