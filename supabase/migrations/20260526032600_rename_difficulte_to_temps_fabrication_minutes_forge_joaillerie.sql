-- Rename colonne `difficulte` -> `temps_fabrication_minutes` sur objets_forge et objets_joaillerie.
-- La colonne stockait deja le temps de fabrication en minutes ; le nom etait trompeur.
-- Recrée vue_personnage_creation_complet pour exposer la nouvelle cle JSON.
-- Resout dette DETTE-RENOMMER-DIFFICULTE-EN-TEMPS.

-- 1. Rename colonnes (idempotent via garde DO block)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_schema='public' AND table_name='objets_forge' AND column_name='difficulte') THEN
    ALTER TABLE public.objets_forge RENAME COLUMN difficulte TO temps_fabrication_minutes;
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_schema='public' AND table_name='objets_joaillerie' AND column_name='difficulte') THEN
    ALTER TABLE public.objets_joaillerie RENAME COLUMN difficulte TO temps_fabrication_minutes;
  END IF;
END $$;

-- 2. Recréer vue_personnage_creation_complet
-- Seules modifs vs def precedente : `'difficulte', X.difficulte` -> `'temps_fabrication_minutes', X.temps_fabrication_minutes`
-- dans les 2 jsonb_build_object (forge et joaillerie).
