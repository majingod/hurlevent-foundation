...
const AdminCompetencesMaitre = () => {
  ...
  const enAttente = competences?.filter((c) => c.statut_maitre === "en_attente") ?? [];
  const approuvees = competences?.filter((c) => c.statut_maitre === "approuvee") ?? [];
  const rejetees = competences?.filter((c) => c.statut_maitre === "rejetee") ?? [];
  ...
  return (
    <div className="container max-w-6xl py-8 space-y-6">
      <div>
        <h1 className="font-heading text-3xl font-bold text-primary">Approbations maître</h1>
        <p className="text-muted-foreground mt-1">
          {enAttente.length} en attente • {approuvees.length} approuvées • {rejetees.length} rejetées
        </p>
      </div>
      ...
      {enAttente.length > 0 && (
        <Card className="border-yellow-500/30 bg-yellow-500/5">
          <CardHeader>
            <CardTitle className="text-base text-yellow-700">En attente d'approbation ({enAttente.length})</CardTitle>
          </CardHeader>
